...
def train_and_evaluate(model, train_dataloader, val_dataloader, optimizer, criterion, metrics, opt, val_fold):
    ...
    for epoch in range(opt.num_epochs):
        ...
        if best_val_acc==1.0:
            ...
            return best_train_acc, best_val_acc, best_auc            
...
if __name__ == '__main__':
    ...
    if opt.features == 'gaze':
        opt.feature_dim = 2
    elif opt.features == 'aus': 
        opt.feature_dim = 17
    else:
        opt.feature_dim = 19
    ...
    for i in range(folds_num):
        ...
        train_acc, val_acc, val_auc = train_and_evaluate(model, train_dataloader, val_dataloader, optimizer, criterion, metrics, opt, i)
        accs_train.append(train_acc)
        accs_val.append(val_acc)
        aucs.append(val_auc)
        
    logging.info("-- {} - {} epochs\n- Train_Acc: {}\n- Val Acc: {}\n- Val AUC: {}".format(opt.name, opt.num_epochs, accs_train, accs_val, aucs) )
    logging.info("### Av. Train_Acc: {:.4f} --- Av. Val_Acc: {:.4f} --- Av. AUC: {:.4f}".format( np.mean(accs_train), np.mean(accs_val), np.mean(aucs)) )
    logging.info("###########")
