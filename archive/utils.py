...
class Options():
    def initialize(self, parser):
        ...
        parser.add_argument('--splits',         type=str, default='./data/splits-videos_10.txt', help='text file listing cross-validation splits')
        ...
        parser.add_argument('--features',       type=str,   default='faus+gaze', help='gaze | aus | faus+gaze')
...
def get_dataset(opt, file_list):
    ...
    if opt.setting == 'openface':    
        dataset = RLT(opt.dataroot, file_list, opt.labels, opt.num_frames, opt.time_stride, opt.features)
    else:
